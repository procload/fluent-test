...
test.describe('Radio', () => {
    ...
    test('should set ARIA attributes to match the state', async () => {
        ...
        await element.evaluate((node) => (node.checked = true));
        await expect(element).toHaveAttribute('aria-checked', 'true');
        await element.evaluate((node) => (node.checked = false));
        await expect(element).toHaveAttribute('aria-checked', 'false');
        // Required
        await expect(element).toHaveAttribute('aria-required', 'false');
        await element.evaluate((node) => (node.required = true));
        await expect(element).toHaveAttribute('aria-required', 'true');
        await element.evaluate((node) => (node.required = false));
        await expect(element).toHaveAttribute('aria-required', 'false');
        // Disabled
        await expect(element).toHaveAttribute('aria-disabled', 'false');
        await element.evaluate((node) => (node.disabled = true));
        await expect(element).toHaveAttribute('aria-disabled', 'true');
        await element.evaluate((node) => (node.disabled = false));
        await expect(element).toHaveAttribute('aria-disabled', 'false');
    });
    ...
    test('should initialize to the provided value attribute if set pre-connection', async () => {
        ...
        await element.evaluate((node) => node.setAttribute('value', 'foo'));
    ...
    test('should fire an event when spacebar is pressed', async () => {
        ...
        const [wasPressed] = await Promise.all([
            element.evaluate((node) => new Promise(resolve => node.addEventListener('keydown', () => resolve(true), {
                once: true,
            }))),
