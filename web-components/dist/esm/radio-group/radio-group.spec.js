...
test.describe('Radio', () => {
    ...
    test('should set and retrieve the `stacked` property correctly', async () => {
        ...
        const isStackedTrue = await element.evaluate((node) => node.stacked);
        expect(isStackedTrue).toBe(true);
        await element.evaluate((node) => {
            node.stacked = false;
        });
        const isStackedFalse = await element.evaluate((node) => node.stacked);
    ...
    test('should set a matching class on the `positioning-region` when an orientation is provided', async () => {
        ...
        await element.evaluate((node, RadioGroupOrientation) => {
    ...
    test('should set the `aria-disabled` attribute equal to the `disabled` property', async () => {
        ...
        const hasAriaDisabledInitially = await element.evaluate((node) => node.hasAttribute('aria-disabled'));
    ...
    test('should set the `aria-readonly` attribute equal to the `readonly` property', async () => {
        ...
        const hasAriaReadOnly = await element.evaluate((node) => node.hasAttribute('aria-readonly'));
    ...
    test('should NOT set a default `aria-disabled` value when `disabled` is not defined', async () => {
        ...
        const hasAriaDisabled = await element.evaluate((node) => node.hasAttribute('aria-disabled'));
    ...
    test('should NOT modify child radio elements disabled state when the `disabled` attribute is present', async () => {
        ...
        const hasDisabledAttribute = await element.evaluate((node) => node.hasAttribute('disabled'));
        ...
        const expectedFirst = await firstRadio.evaluate(node => node.hasAttribute('disabled'));
        const expectedSecond = await secondRadio.evaluate(node => node.hasAttribute('disabled'));
        const expectedThird = await thirdRadio.evaluate(node => node.hasAttribute('disabled'));
        expect(await firstRadio.evaluate(radio => radio.hasAttribute('disabled'))).toEqual(expectedFirst);
        expect(await secondRadio.evaluate(radio => radio.hasAttribute('disabled'))).toEqual(expectedSecond);
        expect(await thirdRadio.evaluate(radio => radio.hasAttribute('disabled'))).toEqual(expectedThird);
        element.evaluate(node => node.setAttribute('disabled', ''));
        const hasDisabledAttributeAfter = await element.evaluate((node) => node.hasAttribute('disabled'));
    ...
    test('should NOT be focusable when disabled', async () => {
        ...
        const isDisabled = await element.evaluate((node) => node.hasAttribute('disabled'));
        expect(isDisabled).toBe(true);
        await first.focus();
        await expect(first).toBeFocused();
        await first.press('Tab');
        await expect(second).toBeFocused();
        expect(await element.evaluate(node => node.getAttribute('tabindex') === '-1')).toBeTruthy();
    });
      ...
      `;
        ...
        for (let i = 0; i < radioItemsCount; i++) {
            ...
            const isClickable = await page.evaluate(el => {
                ...
                return el === null || el === void 0 ? void 0 : el.dispatchEvent(event);
    ...
    test('should NOT set `tabindex` of 0 to a child radio if its value does not match the radiogroup `value`', async () => {
        ...
        expect(await radios.evaluateAll(radios => radios.every(radio => radio.getAttribute('tabindex') === '-1'))).toBeTruthy();
    });
    ...
    test('should mark only the last radio defaulted to checked as checked', async () => {
        ...
        expect(await radios.evaluateAll(radios => radios.filter(radio => radio.checked).length)).toBe(1);
        await expect(radios.nth(0)).not.toBeChecked();
    ...
    test('should allow resetting of elements by the parent form', async () => {
        ...
        await radios.nth(2).evaluate(node => {
            node.checked = true;
        });
        await expect(radios.nth(0)).not.toBeChecked();
        await expect(radios.nth(1)).not.toBeChecked();
